...
class MixingEmitter(BaseEmitter):
    ...
    def ask(
        ...
        merged_genes = jax.tree_util.tree_map(
            lambda a, b: jnp.concatenate([a, b], axis=0), pop_a.genes, pop_b.genes
        )

        merged_info = dict(pop_a.info)
        if "descriptors" in pop_a.info and "descriptors" in pop_b.info:
            merged_info["descriptors"] = jnp.concatenate(
    ...
    def tell(
        ...
        next_state_a = self.emitter_a.tell(
            state_a_in, repertoire, pop_a, fitnesses[:size_a], desc_a, k1
        )
        next_state_b = self.emitter_b.tell(
            state_b_in, repertoire, pop_b, fitnesses[size_a:], desc_b, k2
        )
