...
class TensorNeatMutationEmitter(AtomicEmitter):
    ...
    def _emit_one(
        ...
        keys: chex.Array, 
        p_n: chex.Array, p_c: chex.Array, 
        new_node_key: chex.Array, 
        new_conn_markers: chex.Array
    ) -> Tuple[chex.Array, chex.Array]:
        # Mutation only
        mut_n, mut_c = self.genome.execute_mutation(state.tn_state, keys[0], p_n, p_c, new_node_key, new_conn_markers)
        return mut_n, mut_c
...
class TensorNeatCrossoverEmitter(AtomicEmitter):
    ...
    def _sample_parents(self, state: Optional[TensorNeatEmitterState], repertoire: Any, keys: chex.Array) -> Tuple[Any, dict, TensorNeatEmitterState]:
        ...
        return parents_tuple, metadata_dict, state
    ...
    def _emit_one(
        ...
        # Crossover only
        cx_n, cx_c = self.genome.execute_crossover(state.tn_state, keys[0], p1_n, p1_c, p2_n, p2_c)
        return cx_n, cx_c
