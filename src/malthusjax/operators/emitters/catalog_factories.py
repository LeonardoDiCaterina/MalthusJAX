...
def build_qdax_replica_emitter(**kwargs: Any) -> QDAXReplicaMixingEmitter:
    ...
    # Bind kwargs
    mut_fn = lambda x, key: mut_base_fn(x, key, **mut_kwargs)
    var_fn = lambda x1, x2, key: var_base_fn(x1, x2, key, **var_kwargs)
...
def build_genetic_mixing_emitter(**kwargs: Any) -> Any:
    ...
    sub_kwargs = {k: v for k, v in kwargs.items() if k not in ["mutation", "crossover", "variation_percentage", "batch_size", "genome_length", "bounds"]}
