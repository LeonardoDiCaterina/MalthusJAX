...
class GeneticMixingEmitter(BaseEmitter):
    ...
    @property
    def num_keys_per_atomic_operation(self) -> int:
        return max(self.mutation.num_keys_per_atomic_operation, self.crossover.num_keys_per_atomic_operation)
    ...
    def ask(self, state: Optional[EmitterState], repertoire: Any, keys: chex.Array, generation: int = 0, params: Any = None) -> Tuple[BasePopulation, Optional[EmitterState]]:
        ...
        cross_pop = self.crossover(cross_keys, p1_pop, p2_pop, config=self.genome_config, generation=generation)
        ...
        blended_genes = mask_expanded * cross_pop.genes.values + (1.0 - mask_expanded) * mut_pop.genes.values
