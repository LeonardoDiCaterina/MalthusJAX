...
class RingTopologyIsland(BaseIslandModel[E]):
    ...
    def migrate(self, key: chex.PRNGKey, multi_pop: BasePopulation) -> BasePopulation:
        ...
        elites_genes = jax.tree_util.tree_map(
            lambda arr: jax.vmap(gather_elites)(arr, elite_indices),
            multi_pop.genes
        )
        elites_fitness = jax.vmap(gather_elites)(multi_pop.fitness, elite_indices)
        
        # 2. Shift elites one island to the right
        migrant_genes = jax.tree_util.tree_map(lambda arr: jnp.roll(arr, shift=1, axis=0), elites_genes)
...
class FullyConnectedIsland(BaseIslandModel[E]):
    ...
    def migrate(self, key: chex.PRNGKey, multi_pop: BasePopulation) -> BasePopulation:
        ...
        flat_genes = jax.tree_util.tree_map(
            lambda x: x.reshape((-1,) + x.shape[2:]), elites_genes
        )
        ...
        def inject_shuffled(arr, idx, shuffled_pool, island_idx):
            ...
            island_migrants = jax.lax.dynamic_slice_in_dim(shuffled_pool, start, self.num_migrants, axis=0)
            return arr.at[idx].set(island_migrants)
            
        island_indices = jnp.arange(self.num_islands)
        final_genes = jax.tree_util.tree_map(
            lambda arr, s_arr: jax.vmap(inject_shuffled, in_axes=(0, 0, None, 0))(arr, worst_indices, s_arr, island_indices),
            multi_pop.genes, shuffled_genes
        )
        final_fitness = jax.vmap(inject_shuffled, in_axes=(0, 0, None, 0))(multi_pop.fitness, worst_indices, shuffled_fitness, island_indices)
