...
def _get_native_fast_kernel(params, compile_jit: bool = True, return_history: bool = True):
    def _evolve_flat(engine, initial_state, flat_state):
        def flat_step(carry, _):
            ...
            sel_op = (
                initial_state.operators.selection.set_num_selections(params.pop_size)
                if hasattr(initial_state.operators.selection, "set_num_selections")
                else initial_state.operators.selection
            )

            dummy_pop = initial_state.population.replace(
                genes=initial_state.population.genes.replace(values=c_values), fitness=c_fitness
            )
            ...
            offspring_values = (
                offspring_values.reshape((-1,) + offspring_values.shape[2:])
                if offspring_values.ndim > 2
                else offspring_values
            )
            ...
            if sel_op.n_elites > 0:
                ...
                mutant_values = jax.lax.dynamic_update_slice(
                    mutant_values, elite_values, (0,) * mutant_values.ndim
                )
...
class NativeFastEngine(GeneticEngine):
    ...
    def run(
        ...
        if verbose:
            print(
                f"Starting NativeFast evolution: {params.num_generations} generations, pop size {params.pop_size}"
            )

        evolve_fn = _get_native_fast_kernel(
            params, compile_jit=compile, return_history=return_history
        )
