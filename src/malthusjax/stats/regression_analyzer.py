...
from malthusjax.stats.core import RegressionDataset, OLSResult
...
class OLSRegressionAnalyzer:
    ...
    def analyze_suite(
                ...
                for var in self.spec.dependent_vars:
                    ...
                    try:
                        ...
                        row = {
                            ...
                            "beta_3 (Interaction)": ols_res.coefficients.get("interaction_treatment_log_D", np.nan),
                            "beta_3_pval": ols_res.p_values.get("interaction_treatment_log_D", np.nan),
                        }
                        
                        # Add robust p-values for the interaction term
                        for hc in self.spec.robust_cov_types:
                            robust_pval = ols_res.robust_p_values.get("interaction_treatment_log_D", {}).get(hc, np.nan)
        ...
        if not df_pivot.empty and self.spec.apply_multiple_testing:
            ...
            cols = ["Target", "Benchmark", "Dependent_Var"] + [c for c in df_pivot.columns if c not in ["Target", "Benchmark", "Dependent_Var"]]
