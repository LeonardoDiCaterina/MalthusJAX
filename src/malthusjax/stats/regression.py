...
from malthusjax.stats.registry import get_diagnostic
...
def fit_ols(dataset: RegressionDataset) -> OLSResult:
    ...
    for k in keys:
        if dataset.X[k].shape != y.shape:
            raise ValueError(f"Feature '{k}' shape {dataset.X[k].shape} does not match target shape {y.shape}")
    ...
    # Run diagnostics
    diagnostics = [
        breusch_pagan(model.resid, df_X.values),
        shapiro_wilk(model.resid)
    ]
