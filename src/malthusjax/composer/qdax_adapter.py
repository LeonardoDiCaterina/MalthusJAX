            ...
            if not is_pkg:
                try:
                    ...
                    for cls_name, cls_obj in inspect.getmembers(mod, inspect.isclass):
                        if (
                            cls_obj.__module__ == mod.__name__
                            and hasattr(cls_obj, "init")
                            and hasattr(cls_obj, "update")
                        ):
...
def _qdax_malthusjax_eval(
    ...
    def scoring_fn(
        ...
        pop = RealPopulation(
            genes=genes, fitness=jnp.zeros(genotypes.shape[0]), config=genome_config
        )
        ...
        # QDAX always maximizes.
        # If user wants to maximize, we pass it directly.
        # If user wants to minimize, we negate it.
...
QDAX_METRICS = [
    MetricSpec(name="best_fitness", source="max_fitness", is_objective_value=True),
    MetricSpec(name="qd_score", source="qd_score", is_objective_value=True),
...
def build_qdax_engine(
    ...
    **kwargs: Any,
    ...
    params = {
        ...
        if init_variables is not None
        else kwargs.get("genome_length", 1),
    }
