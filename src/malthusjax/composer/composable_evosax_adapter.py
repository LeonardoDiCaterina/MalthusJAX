...
@adapter(
    ...
    metrics_catalog=EVOSAX_METRICS,
)
class ComposableEvosaxAdapter:
    ...
    def _adapter_step(
        ...
        # 3. Process metrics
        # EvoSAX minimizes. If we want to maximize, we must flip the fitness provided to tell
        evosax_fitness = jnp.where(self.maximize, -fitness, fitness)

        # EvoSAX natively minimizes tell_fitness (evosax_fitness).
        # We compute manual metrics in this minimized space so UniversalAdapterEngine
        # flips them back to user space properly if maximize=True.
        mean_fit = jnp.mean(evosax_fitness)
        std_fit = jnp.std(evosax_fitness)
        best_fit_in_gen = jnp.min(evosax_fitness)

        # 4. Tell
        state, metrics = strategy.tell(key_tell, population, evosax_fitness, state, params)

        # Pack metrics without overwriting the running 'best_fitness'
        metrics = dict(metrics)
        metrics["best_fitness_in_generation"] = best_fit_in_gen
...
def build_composable_evosax_engine(
    ...
    **kwargs: Any,
) -> ComposableEvosaxAdapter:
    """Build an :class:`ComposableEvosaxAdapter` from high-level specs."""
    if "num_generations" in kwargs:
        generations = int(kwargs.pop("num_generations"))

    if evaluator is None:
        raise ValueError("build_composable_evosax_engine requires an evaluator argument")

    if fitness_spec is not None and isinstance(evaluator, OptimizationEvaluator):
    ...
    if isinstance(evaluator, OptimizationEvaluator):
        ...
        problem = None
        problem_state = None
        # Infer dimensions from the MalthusJAX evaluator's genome config or interpreter
        if hasattr(evaluator, "interpreter") and getattr(evaluator.interpreter, "num_params", None):
            num_dims = evaluator.interpreter.num_params
        elif hasattr(evaluator, "config"):
            if hasattr(evaluator.config, "genome_config") and hasattr(
                ...
                inferred_dims = int(kwargs.get("num_dims", kwargs.get("genome_length")) or 0)
                if inferred_dims > 1:
                    num_dims = inferred_dims
                else:
                    num_dims = inferred_dims
            else:
                raise ValueError(
            ...
            inferred_dims = int(kwargs.get("num_dims", kwargs.get("genome_length")) or 0)
            # factory passes num_dims=1 by default, which is wrong if we can't infer it
            num_dims = inferred_dims
        else:
    ...
    return ComposableEvosaxAdapter(
        strategy=strategy,
        params=params,
        problem=problem,
