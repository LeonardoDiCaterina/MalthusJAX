...
class TensorNeatQDEvaluator(BaseQDEvaluator[TensorNeatGenome, TensorNeatEvaluatorConfig, Any]):
    ...
    def create(
        ...
        problem: Any,
        forward_fn: Any,
        config: Optional[TensorNeatEvaluatorConfig] = None,
    ) -> "TensorNeatQDEvaluator":
        """Constructs the evaluator from TensorNEAT components."""
        if config is None:
            config = TensorNeatEvaluatorConfig()
        return cls(
            config=config, data=None, algorithm=algorithm, problem=problem, forward_fn=forward_fn
        )
    ...
    def evaluate_qd(self, genome: TensorNeatGenome) -> Tuple[chex.Numeric, chex.Array]:
        ...
        raise NotImplementedError(
            "TensorNeatQDEvaluator relies on vectorized batch evaluation via evaluate_population."
        )

    def evaluate_population(
        self, population: BasePopulation[TensorNeatGenome]
    ) -> QDPopulation[TensorNeatGenome]:
        ...
        return QDPopulation(
            genes=population.genes, fitness=fitnesses, config=population.config, info=new_info
        )
