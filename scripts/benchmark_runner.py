...
def run_suite(toml_path: str, force_smoke: bool = False) -> None:
    ...
    for i, coord in enumerate(grid_coords, 1):
            ...
            for k, v in p_kwargs.items():
                if isinstance(v, str):
                    formatted_val = v.format(
                        pop_size=P, genome_length=D, generations=G, elite_k=max(2, int(P / 6))
                    )
                    formatted_kwargs[k] = (
                        int(formatted_val) if formatted_val.isdigit() else formatted_val
                    )
                else:
        ...
        try:
            ...
            all_results.append(
                {"experiment": experiment_name, "status": "success", "wall_time": wall_time}
            )
            ...
            traceback.print_exc()
            all_results.append({"experiment": experiment_name, "status": "error", "error": str(e)})
...
def main() -> None:
    ...
    parser.add_argument(
        "--smoke", action="store_true", help="Run a quick smoke test (2 coords, 3 seeds)"
    )
