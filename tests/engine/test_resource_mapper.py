...
def test_compute_resource_map_basic():
    ...
    resource_map = compute_resource_map(selection, crossover, mutation, genome_config, pop_size)
...
def test_compute_resource_map_different_genome_types():
    ...
    rmap_binary = compute_resource_map(selection, crossover, mutation, binary_config, 100)
    assert rmap_binary.genome_shape == (50,)

    # Real genome
    real_config = RealGenomeConfig(shape=(20,), bounds=(-10.0, 10.0))
    rmap_real = compute_resource_map(selection, crossover, mutation, real_config, 100)
    assert rmap_real.genome_shape == (20,)

    # Categorical genome
    cat_config = CategoricalGenomeConfig(shape=(5,), num_categories=15)
    rmap_cat = compute_resource_map(selection, crossover, mutation, cat_config, 100)
...
def test_resource_map_get_key_slice():
    ...
    resource_map = compute_resource_map(selection, crossover, mutation, genome_config, 100)
...
def test_get_resource_summary():
    ...
    resource_map = compute_resource_map(selection, crossover, mutation, genome_config, 50)
...
def test_genetic_engine_resource_map_consistency():
    """Test that resource map is consistent across multiple state
    initializations with same engine."""
    from malthusjax.core.fitness.binary_evaluators import BinarySumConfig, BinarySumEvaluator
