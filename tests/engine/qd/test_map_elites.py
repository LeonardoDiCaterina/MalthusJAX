...
class DummyRealEvaluator(BaseQDEvaluator):
    def evaluate_population(self, population: RealPopulation) -> RealPopulation:
        ...
        return population.replace(fitness=fitnesses, info={"descriptors": descriptors})
...
def test_map_elites_engine_fold_in():
    ...
    mut_emitter = GeneticMutationEmitter(
        _batch_size=8, mutation=mutation, genome_config=genome_config
    )
    cx_emitter = GeneticCrossoverEmitter(
        _batch_size=8, crossover=crossover, genome_config=genome_config
    )
    ...
    engine_params = MapElitesEngineParams(
        pop_size=16, num_generations=10, key_derivation="fold_in", maximize=True
    )
...
def test_map_elites_engine_split():
    ...
    engine_params = MapElitesEngineParams(
        pop_size=16, num_generations=2, key_derivation="split", maximize=True
    )
