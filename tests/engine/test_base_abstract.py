...
from malthusjax.engine.base import (
    AbstractEngine, AbstractEngineParams, AbstractEvolutionState, AbstractGenerationOutput,
    compute_unroll_num, validate_engine_params, _get_evolution_kernel
)
...
class DummyEngineWithInterfaces(AbstractEngine):
    def init_state(self, rng_key): pass
    def step(self, state):
        return state, AbstractGenerationOutput(jnp.array(0.), jnp.array(0.), jnp.array(0.), jnp.array(0))
    def ask(self, state):
        return self, None
...
def test_engine_base_coverage():
    ...
    assert AbstractGenerationOutput.get_kpi_names() == ["best_fitness", "mean_fitness", "std_fitness", "generation"]
    ...
    # Run loops
    state = AbstractEvolutionState(population=None, best_genome=None, generation=0, best_fitness=jnp.array(0.), rng_key=jnp.array([0,0]))
    ...
    final_state, hist, time_spent = engine_with_interfaces.run(state, time_it=True, compile=False, verbose=True)
