...
def test_ring_topology(multi_pop):
    topology = RingTopologyIsland(engine=MockEngine(), num_islands=3, migration_interval=1, num_migrants=1)
...
def test_fully_connected_topology(multi_pop):
    topology = FullyConnectedIsland(engine=MockEngine(), num_islands=3, migration_interval=1, num_migrants=1)
    key = jax.random.PRNGKey(0)
    migrated_pop = topology.migrate(key, multi_pop)
    assert migrated_pop.genes.shape == (3, 2, 1)
