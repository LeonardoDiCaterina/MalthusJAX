...
def test_constant_returns_initial():
    for gen in [0, 5, 10]:
        result = compute_scheduled_strength(ScheduleType.CONSTANT, gen, 10, initial_strength=0.5)
        assert abs(float(result) - 0.5) < 1e-5


def test_linear_decay_endpoints():
    s0 = compute_scheduled_strength(
        ScheduleType.LINEAR_DECAY, 0, 100, initial_strength=1.0, final_strength=0.1
    )
...
def test_cosine_anneal_endpoints():
    s0 = compute_scheduled_strength(
        ScheduleType.COSINE_ANNEAL, 0, 100, initial_strength=1.0, final_strength=0.0
    )
...
def test_different_schedules_complete(make_engine, prng_key, schedule_type):
    engine = make_engine(
        schedule_type=schedule_type, pop_size=25, genome_shape=(2,), num_generations=10
    )
    state = engine.init_state(prng_key)
    for _ in range(3):
        state, _ = engine.step(state)
