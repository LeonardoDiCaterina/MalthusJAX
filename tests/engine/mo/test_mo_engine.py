...
def test_mo_engine_integration():
    ...
    emitter = MixingEmitter(
        emitter_a=GeneticMutationEmitter(
            mutation=mutation, genome_config=genome_config, _batch_size=pop_size // 2
        ),
        emitter_b=GeneticCrossoverEmitter(
            crossover=crossover, genome_config=genome_config, _batch_size=pop_size - (pop_size // 2)
        ),
    )

    # Setup engine
    engine_params = MOEngineParams(pop_size=pop_size, num_generations=num_generations)
