...
from malthusjax.operators.mutation.real import DitherMutation
from malthusjax.core.fitness.bbob_evaluator import BBOBEvaluator, BBOBConfig
from malthusjax.engine.base import AbstractEngineParams, AbstractEvolutionState
...
class TestLevel3Engine(unittest.TestCase):
    def setUp(self):
        ...
        self.engine_params = AbstractEngineParams(
            pop_size=self.pop_size,
            elitism=0,  # DE doesn't use elitism in the same way
            num_generations=self.generations
        )
        
        # 2. Operators (DE-specific)
        ...
        # DE uses BinomialCrossover and DitherMutation
        self.crossover = BinomialCrossover(
            crossover_rate=0.9
        )
        self.mutation = DitherMutation(
            low=0.2,
            high=0.8,
            clip=True
        )
        ...
        # 3. Engine
        self.engine = DifferentialEvolutionEngine(
    ...
    def test_01_init_state_baking(self):
        ...
        self.assertIsInstance(state, AbstractEvolutionState)
        ...
        # DE operators don't have the same input_length structure as GA
        # Just verify they exist
        ops = state.operators
        self.assertIsNotNone(ops.crossover)
        self.assertIsNotNone(ops.mutation)
    ...
    def test_05_odd_population_size(self):
        """Test that DE handles odd population sizes correctly."""
        ...
        # Verify Output Shape - DE should maintain population size
