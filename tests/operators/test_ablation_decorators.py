"""
Comprehensive test suite for ablation study decorators.

Tests verify that ablation decorators correctly:
1. Override num_keys() to return 1 (bypass budgeting)
2. Override __call__() to split keys internally
3. Produce functionally equivalent results to original operators
4. Maintain proper key management behavior
5. Work with both mutation and crossover operators
"""
...
class TestAblationMutationDecorator:
    """Test suite for @ablation_single_key_mutation decorator."""
    ...
    def test_decorator_overrides_num_keys_binary(self):
        """Ablation decorator should override num_keys to return 1."""
        ...
        # Ablation should always return 1
    ...
    def test_functional_equivalence_binary(self, rng_key, binary_population, binary_config):
        """Ablation mutation should produce equivalent results to original."""
        ...
        ablation_keys = jr.split(key2, ablation_budget)[0] if ablation_budget == 1 else jr.split(key2, ablation_budget)
    ...
    def test_functional_equivalence_real(self, rng_key, real_population, real_config):
        """Ablation mutation should produce equivalent results for real genomes."""
        key1, key2 = jr.split(rng_key)
    ...
    def test_jit_compatibility(self, rng_key, binary_population, binary_config):
        ...
        operator = BitFlipMutation_ablation(
            mutation_rate=0.1, 
            num_offspring=1
        ).set_input_length(len(binary_population))
...
class TestAblationCrossoverDecorator:
    """Test suite for @ablation_single_key_crossover decorator."""
    ...
    def test_decorator_overrides_num_keys_crossover(self):
        ...
        original = BinaryUniformCrossover(
            crossover_rate=0.7, 
            num_offspring=2
        ).set_input_length(4)
    ...
    def test_functional_equivalence_binary_crossover(self, rng_key, binary_populations, binary_config):
...
class TestAblationIntegration:
    """Integration tests for ablation decorators."""
