...
from malthusjax.operators.base_injection import BaseMutation_injection, BaseCrossover_injection
...
class DummyMutation(BaseMutation):
    @property
    def num_keys_per_atomic_operation(self): return 1
    def _mutate_one(self, genome, noise, config, **kwargs): return genome
    def _generate_noise(self, keys, config, generation=0): return keys
...
class DummyCrossover(BaseCrossover):
    ...
    def num_keys_per_atomic_operation(self): return 1
    def _generate_noise(self, keys, config, generation=0): return keys
    def _recombine_one(self, p1, p2, noise, config, **kwargs): return p1
...
class DummySelection(BaseSelection):
    @property
    def num_keys_per_atomic_operation(self): return 1
    def _select(self, keys, fitness, config=None, **kwargs): return jnp.array([0, 1])

def test_base_mutation_methods():
    mutations = [
        (DummyMutation(num_offspring=2).set_input_length(10).set_typed_keys(True).set_max_generations(100), 20),
        (DummyMutation_injection(num_offspring=2).set_input_length(10).set_typed_keys(True).set_max_generations(100), 1)
    ]
...
def test_base_crossover_methods():
    crossovers = [
        (DummyCrossover(num_offspring=2).set_input_length(10).set_typed_keys(True).set_max_generations(100), 20),
        (DummyCrossover_injection(num_offspring=2).set_input_length(10).set_typed_keys(True).set_max_generations(100), 1)
    ]
