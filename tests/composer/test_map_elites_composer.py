...
def test_composer_native_map_elites_tensorneat():
    ...
    emitter = TensorNeatEmitter(
        _batch_size=16, genome=tn_genome, mutation=mutation, crossover=crossover
    )
    ...
    result = composer.quick_run(
        strategy=strategy, objective_function=objective_fn, pop_size=16, generations=2, seeds=[1]
    )
...
def test_native_vs_qdax_benchmarking():
    ...
    pipelines = {
        ...
        "QDAX_MAP_Elites": {
            "strategy": QDAXStrategy(
                strategy_cls="MAPElites", num_descriptors=2, num_centroids=10, mutation_sigma=0.1
            ),
            "fitness": "bbob:rosenbrock:dim=10",
            "pop_size": 20,
        },
    ...
    result = composer.compare(
        pipelines=pipelines, generations=2, seeds=[1], maximize=True, bounds=(-5.0, 5.0)
    )
