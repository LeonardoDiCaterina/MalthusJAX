...
from malthusjax.stats.core import (
    TestResult, TOSTResult, EffectSizeResult,
    StatisticalComparisonResult, StatisticalSuiteResult,
...
def test_suite_to_dict_roundtrip():
    ...
    tost_res = TOSTResult(
        margin=1.0, lower_bound=-1.0, upper_bound=1.0,
        t_stat_lower=2.0, t_stat_upper=-2.0,
        p_value_lower=0.01, p_value_upper=0.01, p_value_max=0.01,
        equivalent=True
    )
...
def test_none_values_in_results():
    spec = StatisticalComparisonSpec(metric_name="score")
    comp = StatisticalComparisonResult(
        label="test", hypothesis_text="shift", n_paired=0,
        wins_left=0, wins_right=0, ties=0,
        left_mean=0.0, right_mean=0.0,
        mean_diff_left_minus_right=0.0, median_diff_left_minus_right=0.0,
        tests={"t": TestResult("t", None, None, "two-sided")},
        tost=None,
        effects=EffectSizeResult(None, None),
        alpha=0.05, decision_pass=None, decision_basis="none",
        metadata={}
    )
    suite = StatisticalSuiteResult(spec=spec, results=[comp])
    d = suite.to_dict()
