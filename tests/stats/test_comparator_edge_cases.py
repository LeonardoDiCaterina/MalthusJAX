...
def test_comparator_identical_arrays():
    ...
    metrics = compare_paired_arrays(
        label="test", left_name="left", right_name="right", left=arr1, right=arr2, spec=spec
    )
...
def test_paired_dataset_from_experiments():
    ...
    from malthusjax.stats.core import HypothesisKind, StatisticalComparisonSpec

    runs_left = [
        RunResult(
            seed=s,
            status="success",
            metrics={"best_fitness": float(s), "initial_fitness": 10.0},
            history=[{"best_fitness": 10.0}],
    ...
    runs_right = [
        RunResult(
            seed=s,
            status="success",
            metrics={"best_fitness": float(s + 1), "initial_fitness": 12.0},
            history=[{"best_fitness": 12.0}],
    ...
    spec = StatisticalComparisonSpec(
        min_paired_seeds=3, include_value_lists=True, include_timing_stats=True
    )
    ...
    spec_opt = StatisticalComparisonSpec(
        min_paired_seeds=3, hypothesis_kind=HypothesisKind.CLOSER_TO_OPTIMUM, optimum_value=0.0
    )
...
def test_paired_dataset_from_artifacts(tmp_path):
    ...
    summary_left = {
        "runs": [
            {"seed": s, "metrics": {"gap_to_optimum": 0.1}, "timings": {"step": 0.05}}
            for s in range(5)
        ]
    }
    summary_right = {
        "runs": [
            {"seed": s, "metrics": {"gap_to_optimum": 0.2}, "timings": {"step": 0.05}}
            for s in range(5)
        ]
    }
...
def test_comparator_extra_branches():
    ...
    exp_l = ExperimentResult(
        name="p1", runs=[RunResult(seed=0, status="success", metrics={"best_fitness": 1.0})]
    )
    exp_r = ExperimentResult(
        name="p2", runs=[RunResult(seed=0, status="success", metrics={"best_fitness": 2.0})]
    )
    ...
    spec_bonf = StatisticalComparisonSpec(
        min_paired_seeds=1, multiple_testing=MultipleTestingPolicy.FDR_BH
    )
    comparator = StatisticalComparator()
    ds1 = PairedMetricDataset(
        "ds1", "p1", "p2", [0], np.array([1.0]), np.array([2.0]), "fitness", "source"
    )
...
def test_comparator_shapiro_normality():
    ...
    spec_parametric = StatisticalComparisonSpec(
        min_paired_seeds=10, include_tests=("paired_t",), alpha=0.05
    )
    ...
    spec_nonparametric = StatisticalComparisonSpec(
        min_paired_seeds=10, include_tests=("wilcoxon",), alpha=0.05
    )
