...
def test_binary_sum_vmap_consistency(binary_population):
    ...
    chex.assert_trees_all_close(manual_fitness, pop_with_fitness.fitness, rtol=0.0, atol=0.0)
...
def test_knapsack_capacity_edge_cases():
    ...
    config_zero = KnapsackConfig(
        maximize=True, weights=weights, values=values, capacity=0.0, penalty_factor=penalty_factor
    )
...
def test_knapsack_factory_determinism_and_properties(rng_key):
    ...
    config = KnapsackEvaluator.create_random_problem(
        rng_key, n_items=n_items, capacity_ratio=capacity_ratio
    )
...
def test_knapsack_zero_weight_items():
    ...
    config_zv = KnapsackConfig(
        maximize=True, weights=weights_zv, values=values_zv, capacity=1.5, penalty_factor=100.0
    )
...
def test_binary_evaluators_jit_consistency(binary_population):
    ...
    chex.assert_trees_all_close(non_jit_result.fitness, jit_result.fitness, rtol=0.0, atol=0.0)
