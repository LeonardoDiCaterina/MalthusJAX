...
To seamlessly map these structural tests to the empirical regression tables presented in **Chapter 5**, the pipelines are explicitly defined as follows:
1. **Selection Ablation (`mjx_ablate_sel_tournament`, `mjx_ablate_sel_elite`):** Evaluates JAX-native Tournament and Elite selection architectures against the wrapped EvoSAX baseline equivalents.
2. **Crossover Ablation (`mjx_ablate_crossover`):** Evaluates MalthusJAX's native Sparse Uniform Crossover against the wrapped EvoSAX Uniform crossover.
3. **Mutation Ablation (`mjx_ablate_mutation`):** Evaluates MalthusJAX's native Sparse Gaussian Mutation against the wrapped EvoSAX Dense Gaussian Mutation.

*(Note: The primary control pipeline against which all these ablations are mathematically regressed is the JAX-native orchestrator, denoted as `mjx_baseline` in the results).*
...
- **Holm-Bonferroni Correction:** Applied systematically across all ablation and representation hypothesis tests to strictly control the Family-Wise Error Rate (FWER) when performing multiple comparative statistical tests, preventing false-positive discoveries.
...
When these formal diagnostics flag severe violations ($p < 0.05$)—particularly heteroskedasticity detected by the Breusch-Pagan test—the automated pipeline strictly rejects standard OLS inference. To mathematically correct for this variance inflation, the model automatically implements **HC3 (MacKinnon and White) Robust Standard Errors**. This guarantees that the baseline OLS $p$-values and confidence intervals governing the interaction term ($\beta_3$) remain asymptotically consistent and unbiased, even under severe topographical scaling stress.
