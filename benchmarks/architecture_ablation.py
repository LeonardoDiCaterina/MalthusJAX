...
from malthusjax.engine.schedules import TrackMetrics, TrackBest
...
def setup_engine(engine_type: str, pop_size: int, gens: int, track_metrics: TrackMetrics, mutation_op: str = "evosax_gaussian:mutation_strength=1.0"):
...
def bench_engine(name: str, engine_instance, key: jax.Array, runs: int, gens: int, return_history: bool) -> float:
    """
...
def run_ablation(pop_size: int, gens: int, runs: int):
    print("=" * 68)
    print(f"  MALTHUSJAX ARCHITECTURAL ABLATION STUDY")
    ...
    t_fast_none = bench_engine("1. NativeFastEngine (No Tracking/History)", e_fast_none, key, runs, gens, False)

    # 2. NativeFastEngine (+ History Tensor)
    e_fast_hist = setup_engine("native_fast", pop_size, gens, TrackMetrics.NONE)
    t_fast_hist = bench_engine("2. NativeFastEngine (+ History Tensor)", e_fast_hist, key, runs, gens, True)

    # 3. NativeFastEngine (+ Hist + Metrics)
    e_fast_metrics = setup_engine("native_fast", pop_size, gens, TrackMetrics.BASIC)
    t_fast_metrics = bench_engine("3. NativeFastEngine (+ Hist + Metrics)", e_fast_metrics, key, runs, gens, True)

    # 4. GeneticEngine (No Tracking/History)
    e_gen_none = setup_engine("ga", pop_size, gens, TrackMetrics.NONE)
    t_gen_none = bench_engine("4. GeneticEngine (No Tracking/History)", e_gen_none, key, runs, gens, False)

    # 5. GeneticEngine (Full Featured Default)
    e_gen_full = setup_engine("ga", pop_size, gens, TrackMetrics.BASIC)
    t_gen_full = bench_engine("5. GeneticEngine (Full Featured Default)", e_gen_full, key, runs, gens, True)

    # 6. GeneticEngine (Batched EvoSAX Mutation)
    e_gen_batched = setup_engine("ga", pop_size, gens, TrackMetrics.BASIC, mutation_op="batched_evosax_gaussian:mutation_strength=1.0")
    t_gen_batched = bench_engine("6. GeneticEngine (Batched EvoSAX Mutation)", e_gen_batched, key, runs, gens, True)

    print("=" * 68)
    print("  OVERHEAD ANALYSIS (relative to previous step)")
    print("=" * 68)
    print(f"  Base EvoSAX:                  {t_evosax:.2f} ms")
    print(f"  + MalthusJAX Abstractions:    {t_fast_none - t_evosax:+.2f} ms  (-> {t_fast_none:.2f} ms)")
    print(f"  + History Tensor Allocation:  {t_fast_hist - t_fast_none:+.2f} ms  (-> {t_fast_hist:.2f} ms)")
    print(f"  + Metric Math (Mean/Std):     {t_fast_metrics - t_fast_hist:+.2f} ms  (-> {t_fast_metrics:.2f} ms)")
    print(f"  + PyTree passing in Scan:     {t_gen_full - t_fast_metrics:+.2f} ms  (-> {t_gen_full:.2f} ms)")
    print(f"  + Batched EvoSAX Operator:    {t_gen_batched - t_gen_full:+.2f} ms  (-> {t_gen_batched:.2f} ms)")
    print("=" * 68)
    ...
    print(f"  TOTAL OVERHEAD (Default):     {total_overhead_default:+.2f} ms ({percent_overhead_default:+.1f}%)")
    print(f"  TOTAL OVERHEAD (Batched):     {total_overhead_batched:+.2f} ms ({percent_overhead_batched:+.1f}%)")
    print("====================================================================")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--runs", type=int, default=50, help="Number of times to run the full evolution for averaging")
